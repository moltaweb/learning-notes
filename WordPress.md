...
To add a setting:
    ...
    create a function to register the setting options, sections and fields, and hook it in 'admin_init'
    register the setting options via the register_setting() function
    add sections via the add_settings_section() function
    add fields to each section via the add_settings_field() function
...
Basically, for each option/setting, we add a hook (action/filter) that does what ispromised



Include JavaScript and CSS

In this video, we build the custom functionality for the "cusotm style"setting
...
General steps:
    ...
    prepare folders and files
        add a languages/ folder
        use the same slug for main folder and file
...
3) the POT file is a template translators will use when translating the plugin
we can generate it with poedit, or with Loco Translate plugin
...
User's data is soterd in the database table "users"
...
To enqeue scripts and styles in the admin area, we use the hook 'admin_enqueue_scripts'

To enqeue scripts and styles in the front site, we use the hook 'wp_enqueue_scripts'

To enqeue scripts and styles in the login page, we use the hook 'login_enqueue_scripts'
...
# WORDPRESS: ACTION AND FILTER GOOKS
...
Creatiing our own hooks is pretty easy with the apply_filters and do_action functions
...
He uses wp_reset_query() to restore the main query. However, official documentation says to use wp_reset_postdata() instead
https://codex.wordpress.org/wp_reset_postdata



GOING FURTHER WITH CUSTOM POST TYPES
...
The basic idea is that if we want to change the place of something, it's a 2 step:
    ...
    remove_action('old_where', 'what');
    add_action('new_where', 'what');


NOTE:
The apply_filter() function defines where we can modify through a filter
...
Steps:
    ...
    in functions.php, add the code that creates the widget area
...
genesis_register_sidebar( array(
    ...
    in front-page.php, add the code that displays the contents of that widget area
...
There are 2 ways to customize the WP loop
    ...
    make some adjustments on the default query
        exclude some categories
        filter by author
        order by name instead of date
        etc.
    create a whole custom query
...
For example, unregister one of the 6 default layouts:
...
Start with hogher level HTML element classes
...
Prepare your theme
    ...
    clean up the code
    validate the code
    test across platforms, devices and browsers
